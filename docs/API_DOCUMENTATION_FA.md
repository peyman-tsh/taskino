...
رویدادهای اعلان فعلی:

- `TASK_ASSIGNED`: هنگام تخصیص Task به کاربر، اعلان جدید برای مسئول ساخته می‌شود.
- `TASK_COMPLETED`: هنگام تکمیل Task، اعلان جدید برای سازنده Task ساخته می‌شود.
- `TASK_STATUS_CHANGED`: هنگام تغییر وضعیت Task، تمام اعلان‌های مرتبط با همان Task به وضعیت جدید به‌روزرسانی و خوانده‌نشده می‌شوند.
- `FIXED_TASK_ASSIGNED`: هنگام ساخت و تخصیص FixedTask، اعلان جدید برای مسئول ساخته می‌شود.
- `FIXED_TASK_COMPLETED`: هنگام تغییر وضعیت FixedTask به `done` توسط مسئول، اعلان جدید برای سازنده FixedTask ساخته می‌شود.

تمام این رویدادها توسط `InternalEventBus` منتشر و داخل `NotificationEventListener` دریافت می‌شوند؛ بنابراین ساخت یا به‌روزرسانی اعلان‌ها به‌صورت asynchronous انجام می‌شود.
