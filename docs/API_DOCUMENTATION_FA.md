...
- موعد مؤثر Task از تاریخ `dueDate` و ساعت `endTime` ساخته می‌شود؛ اگر `endTime` وجود نداشته باشد، خود `dueDate` موعد است.
...
- هنگام ساخت یا seed کردن FixedTask، اگر `nextRunAt` ارسال نشده باشد، نزدیک‌ترین موعد آینده روزانه، هفتگی یا ماهانه با درنظرگرفتن `endTime` به‌صورت خودکار ذخیره می‌شود.
