...
| متد | مسیر | ورودی | پاسخ موفق |
| --- | --- | --- | --- |
| POST | `/api/fixed-tasks/seed/excel` | فقط Manager؛ body: `sourcePath` | `201`؛ ثبت کاربران و FixedTaskهای چهار شیت |
...
این endpoint فقط برای نقش `manager` قابل دسترسی است و فایل Excel چهارشیتی وظایف ثابت را seed می‌کند. مسیر کامل فایل از طریق `sourcePath` ارسال می‌شود. کاربران پوریا یاری‌زاده، سینا اعلایی، اکبر گنجی، امیر گنجی و امیر رضا جلالیان با رمز اولیه `123456` ثبت یا به‌روزرسانی می‌شوند. پوریا مدیر، سینا سرپرست و سایر افراد کارشناس هستند.
...
```json
{
  "sourcePath": "C:\\Users\\Zarnegar\\Downloads\\-1408847228164432127_81294547954101.xlsx"
}
```
