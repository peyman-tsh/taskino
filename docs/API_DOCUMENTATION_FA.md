...
این endpoint برای نقش‌های `manager` و `supervisor` قابل دسترسی است. تمام کارشناسان و سرپرستان را براساس Taskها و FixedTaskهایشان ارزیابی می‌کند. درصد پیشرفت و وضعیت `good` یا `weak` محاسبه و داخل رکورد User ذخیره می‌شود. کاربر زمانی `good` است که تمام FixedTaskها انجام شده و تمام Taskهای معمولی قبل از موعد تکمیل شده باشند.
...
برای هر عضو، اطلاعات هویتی، نقش، فعال‌بودن، امتیاز، درصد پیشرفت، وضعیت عملکرد `good` یا `weak`، زمان آخرین ارزیابی و تعداد Task و FixedTaskهای تخصیص‌یافته و انجام‌شده نمایش داده می‌شود. فیلتر اختیاری `recurrence` با مقادیر `daily`، `weekly` یا `monthly` روی تشخیص اعضا و شمار وظایف آن‌ها اعمال می‌شود.
...
| `performanceStatus` | enum | خیر | `weak` | نتیجه ارزیابی عملکرد: `good` یا `weak` |
