...
| متد | مسیر | ورودی | پاسخ موفق |
| --- | --- | --- | --- |
| POST | `/api/fixed-tasks/seed/excel` | عمومی؛ بدون توکن و body | `201`؛ ثبت کاربران و FixedTaskهای چهار شیت |
...
- هر Task معمولی که قبل یا دقیقاً در `dueDate` تکمیل شود، یک‌بار `۱۰+` امتیاز می‌دهد.
- Task معمولی تکمیل‌شده بعد از `dueDate` یا Task موعدگذشته و انجام‌نشده، یک‌بار `۱۰-` امتیاز می‌دهد.
- هر FixedTask که قبل یا دقیقاً در `nextRunAt` تکمیل شود، یک‌بار `۱۰+` امتیاز می‌دهد.
- FixedTask تکمیل‌شده بعد از `nextRunAt` یا FixedTask موعدگذشته و انجام‌نشده، یک‌بار `۱۰-` امتیاز می‌دهد.
- هنگام ساخت یا seed کردن FixedTask، اگر `nextRunAt` ارسال نشده باشد، پایان بازه روزانه، هفتگی یا ماهانه آن به‌صورت خودکار به‌عنوان موعد بعدی ذخیره می‌شود.
